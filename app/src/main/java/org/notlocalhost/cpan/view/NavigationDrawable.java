...
public class NavigationDrawable extends Drawable implements Animatable {
    ...
    private static final float ARROW_TOP_LINE_ANGLE = 135; // 135
    private static final float ARROW_BOT_LINE_ANGLE = 225; // 225
    ...
    public NavigationDrawable(Context context, int color, Stroke stroke) {
        this(context, color, stroke, DEFAULT_SCALE, DEFAULT_TRANSFORM_DURATION, DEFAULT_PRESSED_DURATION);
    }

    public NavigationDrawable(Context context, int color, Stroke stroke, int transformDuration, int pressedDuration) {
        this(context, color, stroke, DEFAULT_SCALE, transformDuration, pressedDuration);
    }

    public NavigationDrawable(Context context, int color, Stroke stroke, int scale, int transformDuration, int pressedDuration) {
        ...
        this.topPadding = (height - 5 * dip4) / 3;
    ...
    private NavigationDrawable(int color, Stroke stroke, long transformDuration, long pressedDuration,
    ...
    private void initPaint(int color) {
        ...
        iconPaint.setStyle(Style.FILL_AND_STROKE);
    ...
    private void drawMiddleLine(Canvas canvas, float ratio) {
        ...
        canvas.drawCircle(startX, startY, DOT_RADIUS, iconPaint);
        canvas.drawCircle(stopX, stopY, DOT_RADIUS, iconPaint);
     ...
     * Animations
     */
    private Property<NavigationDrawable, Float> transformationProperty
            = new Property<NavigationDrawable, Float>(Float.class, "transformation") {
        @Override
        public Float get(NavigationDrawable object) {
            return object.getTransformationValue();
        }

        @Override
        public void set(NavigationDrawable object, Float value) {
            object.setTransformationValue(value);
        }
    };

    private Property<NavigationDrawable, Float> pressedProgressProperty
            = new Property<NavigationDrawable, Float>(Float.class, "pressedProgress") {
        @Override
        public Float get(NavigationDrawable object) {
    ...
    private final class MaterialMenuState extends ConstantState {
        ...
        public Drawable newDrawable() {
            NavigationDrawable drawable = new NavigationDrawable(
